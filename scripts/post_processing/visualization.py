...
from AB3DMOT_libs.kitti_obj import read_label, Object_3D, Object_vod_gt_tracking_3D
...
def parse_args():
    ...
    parser.add_argument('--read_from_gt_tracking', type=bool, default=False, help='error to be highlighted')
...
def read_gt_label(label_filename):
    ...
    objects = [Object_vod_gt_tracking_3D(line) for line in lines]
    return objects
...
def vis(args):
    ...
    for seq in seq_eval:
        ...
        for frame in in_seq:
            ...
            # load results
            if args.read_from_gt_tracking:
                result_dir = os.path.join(file_path, '../../label_2_with_track_ids','label_2')
                result_tmp = os.path.join(result_dir, '%s.txt' % (frame))  # load the result
                # load the result
                if not is_path_exists(result_tmp):
                    object_res = []
                else:
                    object_res = read_gt_label(result_tmp)
            else:
                result_tmp = os.path.join(result_dir, '%s.txt'%(frame))
                # load the result
